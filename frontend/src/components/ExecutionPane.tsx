...
import { latencyTone } from '../lib/ui';
...
export const ExecutionPane: React.FC<ExecutionPaneProps> = ({ result, isRunning, error }) => {
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden border-t border-zinc-800/80 bg-zinc-950 text-[13px]">
      {/* test tab strip */}
      <div className="flex h-9 shrink-0 items-center gap-1 border-b border-zinc-800/80 px-3">
        <span className="mr-2 font-mono text-[11px] uppercase tracking-widest text-zinc-600">
          tests
        </span>
        {result.test_results.map((tc, idx) => (
          <button
            ...
            className={`flex items-center gap-1.5 rounded px-2 py-1 font-mono text-xs transition cursor-pointer ${
              selectedTab === idx
          ...
          >
            ...
            case {idx + 1}
            {tc.hidden && <span className="text-[10px] text-zinc-600">hidden</span>}
          </button>
        ))}
      </div>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-3">
        ...
        {activeTest && (
          <div className="space-y-2">
            {activeTest.error ? (
              <pre className="rounded-md border border-rose-500/20 bg-rose-500/[0.06] p-3 font-mono text-xs leading-relaxed text-rose-300/90">
                {activeTest.error}
              </pre>
            ) : (
              <div className="grid grid-cols-2 gap-2">
                <div className="rounded-md border border-zinc-800/80 bg-zinc-900/40 p-2.5">
                  ...
                  <div className={`font-mono text-xs ${activeTest.passed ? 'text-emerald-400/90' : 'text-rose-300'}`}>
        ...
        {result.stderr && (
          <div className="mt-3">
            ...
            <pre className="rounded-md border border-zinc-800/80 bg-black/50 p-3 font-mono text-xs leading-relaxed text-rose-300/80">
              {result.stderr}
            </pre>
          </div>
        )}
        ...
        {result.stdout && (
          <div className="mt-3">
            ...
            <pre className="rounded-md border border-zinc-800/80 bg-black/50 p-3 font-mono text-xs leading-relaxed text-zinc-400">
              {result.stdout}
            </pre>
          </div>
        )}
      ...
      {/* status bar */}
      <div className="flex h-8 shrink-0 items-center justify-between border-t border-zinc-800/80 bg-zinc-900/50 px-3 font-mono text-[11px]">
